...
# Read files
tx_counts  <- read.table("Lim_RNASeq/nextflow_output/salmon.merged.transcript_counts.tsv",
                         header = TRUE, sep = "\t", check.names = FALSE, 
                         stringsAsFactors = FALSE)
tx_length  <- read.table("Lim_RNASeq/nextflow_output/salmon.merged.transcript_lengths.tsv",
                         header = TRUE, sep = "\t", check.names = FALSE,
                         stringsAsFactors = FALSE)
tx2gene    <- read.table("Lim_RNASeq/nextflow_output/tx2gene.tsv",
                         header = TRUE, sep = "\t", stringsAsFactors = FALSE)

# Extract transcript IDs from tx column
tx_ids <- tx_counts$tx

# Get sample columns only (exclude tx and gene_id)
sample_cols <- colnames(tx_counts)[!colnames(tx_counts) %in% c("tx", "gene_id")]

# Extract numeric columns and convert to matrices
tx_counts_mat <- as.matrix(tx_counts[, sample_cols])
tx_length_mat <- as.matrix(tx_length[, sample_cols])

# Convert to numeric (if needed - but they should already be numeric)
mode(tx_counts_mat) <- "numeric"
mode(tx_length_mat) <- "numeric"

# Set rownames to transcript IDs
rownames(tx_counts_mat) <- tx_ids
rownames(tx_length_mat) <- tx_ids

# Build tximport object
txi <- list(
  abundance = tx_counts_mat,
  counts    = tx_counts_mat,
  length    = tx_length_mat,
  countsFromAbundance = "no"
)

# Summarize to genes (tx2gene columns should be TXNAME and GENEID based on your file)
# Make sure column names match what tximport expects
colnames(tx2gene) <- c("TXNAME", "GENEID")
txi_gene <- summarizeToGene(txi, tx2gene = tx2gene, ignoreTxVersion = TRUE)

# Your design - match sample order from txi_gene
samples <- colnames(txi_gene$counts)
expDesign <- data.frame(sample = samples)
expDesign$wing_type  <- factor(c("fw","fw","fw","hw","hw","hw","fw","fw","fw","hw","hw","hw"))
expDesign$age        <- factor(c("d3","d3","d3","d3","d3","d3","d6","d6","d6","d6","d6","d6"))
expDesign$wing_age   <- factor(c("fw_d3","fw_d3","fw_d3","hw_d3","hw_d3","hw_d3","fw_d6","fw_d6","fw_d6","hw_d6","hw_d6","hw_d6"))
expDesign$individual <- factor(c("1","2","3","1","2","3","4","5","6","4","5","6"))
...
dds <- DESeqDataSetFromTximport(txi_gene, colData = expDesign, design = ~ wing_age)
dds <- DESeq(dds)
results = results(dds)
summary(results)
